...
#define result_of_type typename std::invoke_result<F,Args ...>::type
...
namespace PSI{
    namespace util{
        ...
        public:
            ...
            auto enqueue(F &&f,Args &&... args) -> std::future<result_of_type>;
        ...
        private:
            ...
            std::condition_variable in_flight_conditons;
            ...
            struct handle_in_flight_decrement
            {
                ...
                ~handle_in_flight_decrement(){
                    ...
                    if(prev == 1){
                        ...
                        tp.in_flight_conditons.notify_all();
                    }
                }
            };
        ...
        inline ThreadPool::ThreadPool(std::size_t threads):pool_size(threads)
        {
        ...
        template <class F, class... Args>
        auto ThreadPool::enqueue(F &&f, Args &&... args) -> std::future<result_of_type>
        {
            using return_type = result_of_type;
            ...
            if (tasks.size() >= max_queue_size){
                std::size_t new_queue_size = max_queue_size<<1;
