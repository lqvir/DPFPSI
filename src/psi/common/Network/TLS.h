...
namespace osuCrypto
{
    ...
    error_code readFile(const std::string& file, std::vector<uint8_t>& buffer);
...
namespace osuCrypto
{
    ...
    struct WolfContext
    {
        ...
        void loadCert(span<uint8_t> data, error_code& ec);

        void loadKeyPairFile(std::string pkPath, std::string skPath, error_code& ec);
        void loadKeyPair(span<uint8_t> pkData, span<uint8_t> skData, error_code& ec);
    ...
    struct WolfSocket : public SocketInterface, public LogAdapter
    {
        ...
        uint64_t mSendBufIdx = 0, mRecvBufIdx = 0;
        uint64_t mSendBT = 0, mRecvBT = 0;
        ...
        void setDHParam(span<uint8_t> paramData, error_code& ec);
        ...
        {
            //lout << "in recv cb with " << std::hex << uint64_t(ctx) << std::endl;
        ...
        {
            //lout << "in send cb with " << std::hex << uint64_t(ctx) << std::endl;
            WolfSocket& sock = *(WolfSocket*)ctx;
            assert(sock.mSSL == ssl);
    ...
    extern std::array<uint8_t, 5010> sample_ca_cert_pem;
    extern std::array<uint8_t, 0x26ef> sample_server_cert_pem;
    extern std::array<uint8_t, 0x68f> sample_server_key_pem;
    extern std::array<uint8_t, 0x594> sample_dh2048_pem;
