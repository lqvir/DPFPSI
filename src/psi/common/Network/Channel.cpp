...
#define IF_LOG(m) 
    ...
    ChannelBase::ChannelBase(IOService& ios, SocketInterface* sock)
        :
        mIos(ios),
        mWork(ios, "Channel: SocketInterface." + std::to_string((uint64_t)sock) ),
        mChannelRefCount(1),
        mHandle(sock),
        ...
        boost::asio::dispatch(mStrand, [this, ec]{
            ...
            else
            {
                ...
                if(mChl->mSession->mMode == SessionMode::Client)
                {
                    ...
                    mTLSSock->async_send(buf, [this](const error_code& ec, uint64_t bt){
                        finalize(std::move(mTLSSock), ec);
                    });
                } 
                else 
                {
                    ...
                    mTLSSock->async_recv(buf, [this](const error_code& ec_, uint64_t bt){
                        auto ec = ec_;
    ...
    void StartSocketOp::asyncConnectToServer()
    {
        ...
        auto count = static_cast<uint64_t>(mBackoff) * 100;
    ...
    void StartSocketOp::recvServerMessage()
    {
        ...
        sock.async_receive(buffer, [this](const error_code& ec, uint64_t bytesTransferred) {
    ...
    void StartSocketOp::sendConnectionString()
    {
        ...
        *(details::size_header_type*)mSendBuffer.data()
        ...
        IF_LOG(mChl->mLog.push("Success: async connect to server. ConnectionString = " \
            + str + " " + std::to_string((uint64_t) & *mChl->mHandle)));

        auto buffer = boost::asio::buffer((char*)mSendBuffer.data(), mSendBuffer.size());
        auto& sock = mSock->mSock;;

        sock.async_send(buffer, [this](const error_code& ec, uint64_t bytesTransferred) {
            boost::asio::dispatch(mStrand, [this, ec, bytesTransferred] {
    ...
    void PSI::StartSocketOp::retryConnect(const error_code& ec)
    {
        ...
        else
        {
            ...
            auto count = static_cast<uint64_t>(mBackoff);
            mTimer.expires_from_now(boost::posix_time::millisec(count));
            mBackoff = std::min(mBackoff * 1.2, 1000.0);
            if (mBackoff >= 1000.0)
            {
    ...
    std::string PSI::Channel::commonName()
    {
        if(mBase)
    ...
    void ChannelBase::asyncPerformRecv()
    {
        ...
        if (mRecvCancelNew == false)
        {
            mRecvQueue.front()->asyncPerform(this, [this]
                (error_code ec, uint64_t bytesTransferred) {
    ...
    void ChannelBase::asyncPerformSend()
    {
        ...
        {
            mSendQueue.front()->asyncPerform(this, [this](error_code ec, uint64_t bytesTransferred) {
    ...
    void ChannelBase::asyncCancel(std::function<void()> completionHandle, bool close)
    {
        ...
        else
        {
            ...
            {
                std::cout << "Warning, asyncCancel() called on a canceling or closing channel " << mSession->mName << " " << mLocalName << std::endl;
            }
            completionHandle();
        }
    }
    ...
    void ChannelBase::asyncClose(std::function<void()> completionHandle)
    {
        ...
        if (stopped() == false)
        {
            ...
            auto lifetime = shared_from_this();
            auto count = std::make_shared<std::atomic<uint32_t>>(2);
        ...
        else
        {
            if (mStatus == Channel::Status::Closing ||
                mStatus == Channel::Status::Canceling)
            {
                std::cout << "Warning, asyncClose() called on a canceling or closing channel." << std::endl;
            }
            completionHandle();
        }
    }
    ...
    void ChannelBase::cancelSendQueue(const error_code& ec)
    {
        ...
        if (!mSendQueue.isEmpty())
        {
            ...
            front->asyncCancel(this, ec,[this, ec](const error_code& ec2, uint64_t bt) {
    ...
    uint64_t Channel::getTotalDataSent() const
    {
    ...
    uint64_t Channel::getTotalDataRecv() const
    {
        std::promise<uint64_t> prom;
        boost::asio::dispatch(mBase->mStrand, [&]() {
