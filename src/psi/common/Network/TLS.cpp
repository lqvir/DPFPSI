...
namespace osuCrypto
{
    ...
    void WolfContext::loadCert(span<uint8_t> data, error_code& ec)
    {
        if (isInit() == false)
            ec = make_error_code(TLS_errc::ContextNotInit);
        else
    ...
    void WolfContext::loadKeyPair(span<uint8_t> pk, span<uint8_t> sk, error_code& ec)
    {
        if (isInit() == false) {
            ec = make_error_code(TLS_errc::ContextNotInit);
            return;
        ...
        uint64_t& bt)
    {
        ...
        async_send(buffers, [&](const error_code& ecc, uint64_t btt) {
            ec = ecc;
            bt = btt;
            prom.set_value(); });
        prom.get_future().get();
    ...
    int WolfSocket::sslRquestSendCB(char* buf, int size)
    {
        ...
        if (mState.hasPendingSend() == false)
        {
            ...
            buffer b(buf, size);
            boost::asio::async_write(mSock, b, [this](const error_code& ec, uint64_t bt) {
    ...
    {
        std::promise<void> prom;
        async_recv(buffers, [&](const error_code& ecc, uint64_t btt) {
            ec = ecc;
            bt = btt;
            prom.set_value(); });
        prom.get_future().get();
    ...
    void osuCrypto::WolfSocket::setDHParam(span<uint8_t> data, error_code& ec)
    {
        ec = wolfssl_error_code(
    ...
    int WolfSocket::sslRquestRecvCB(char* buf, int size)
    {
        ...
        if (mState.hasPendingRecv() == false)
        {
            ...
            buffer b(buf, size);
            boost::asio::async_read(mSock, b, [this](const error_code& ec, uint64_t bt) {
                LOG("data recvd ****");
                boost::asio::dispatch(mStrand, [this, ec, bt]() {
