...
namespace PSI {
    ...
    public:
        ...
        uint64_t getTotalDataSent() const;

        // Returns the amount of data that this channel has sent since it was created or when resetStats() was last called.
        uint64_t getTotalDataRecv() const;
        ...
        //uint64_t getMaxOutstandingSendData() const;
    ...
    public:
        ...
        std::atomic<uint32_t> mChannelRefCount;
        ...
        uint64_t mTotalSentData = 0;
        uint64_t mTotalRecvData = 0;
...
#ifdef ENABLE_NET_LOG
        uint32_t mRecvIdx = 0, mSendIdx = 0;
        Log mLog;
#endif
    ...
    typename std::enable_if<is_container<Container>::value, void>::type Channel::asyncSend(std::unique_ptr<Container> c)
    {
        ...
        assert(channelBuffSize(*c) < uint32_t(-1));
    ...
    template<typename T>
    typename std::enable_if<std::is_pod<T>::value, void>::type
        Channel::asyncSendCopy(const T* bufferPtr, uint64_t length)
    {
        std::vector<uint8_t> bs((uint8_t*)bufferPtr, (uint8_t*)bufferPtr + length * sizeof(T));
