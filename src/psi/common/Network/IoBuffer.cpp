...
namespace PSI
{
    namespace details
    {
        ...
        void FixedRecvBuff::asyncPerform(ChannelBase * base, io_completion_handle&& completionHandle)
        {
            ...
            base->mHandle->async_recv({&base->mRecvBuffer, 1}, [this](const error_code& ec, uint64_t bt1) {
                ...
                if (!ec)
                {
                    ...
                    mBase->mHandle->async_recv({ &mBase->mRecvBuffer , 1 }, [this, bt1](const error_code& ec, uint64_t bt2)
                    {
