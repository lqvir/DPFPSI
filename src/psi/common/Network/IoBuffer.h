...
namespace boost {
    namespace system {
        template <>
        struct is_error_code_enum<PSI::Errc> : true_type {};
    }
}
...
namespace { // anonymous namespace
    ...
    struct osuCryptoErrCategory : boost::system::error_category
    {
        ...
        {
            switch (static_cast<PSI::Errc>(ev))
            {
            case PSI::Errc::success:
                return "Success";
...
namespace PSI
{
    ...
    class ChannelBase;
    using io_completion_handle = std::function<void(const error_code&, uint64_t)>;
...
#define _SILENCE_CXX20_IS_POD_DEPRECATION_WARNING
    ...
    public:
        ...
        uint64_t receivedSize() const
        {
            return mTrueReceiveSize;
        }
    };

    static_assert(is_container<ReceiveAtMost<uint8_t>>::value, "sss");
    static_assert(has_resize<ReceiveAtMost<uint8_t>, void(typename ReceiveAtMost<uint8_t>::size_type)>::value, "sss");
    ...
    public:
        ...
        BadReceiveBufferSize(const std::string& what, uint64_t length)
            :
    ...
    template<typename T>
    inline uint8_t* channelBuffData(const T& container) { return (uint8_t*)container.data(); }

    template<typename T>
    inline uint64_t channelBuffSize(const T& container) { return container.size() * sizeof(typename  T::value_type); }

    template<typename T>
    inline bool channelBuffResize(T& container, uint64_t size)
    {
    ...
    namespace details
    {
        ...
        using size_header_type = uint32_t;
        ...
        public:
            ...
            BasicSizedBuff(const uint8_t* data, uint64_t size)
                : mHeaderSize(size_header_type(size))
                , mBuff{ (uint8_t*)data,  std::span<uint8_t>::size_type(size) }
            {
                assert(size < std::numeric_limits<size_header_type>::max());
            }

            void set(const uint8_t* data, uint64_t size)
            {
                assert(size < std::numeric_limits<size_header_type>::max());
                mBuff = { (uint8_t*)data, std::span<uint8_t>::size_type(size) };
            }

            inline uint64_t getHeaderSize() const { return mHeaderSize; }
            inline uint64_t getBufferSize() const { return mBuff.size(); }
            inline uint8_t* getBufferData() { return mBuff.data(); }
        ...
        public:
            ...
            FixedSendBuff(const uint8_t* data, uint64_t size)
        ...
        public:
            ...
            FixedRecvBuff(const uint8_t* data, uint64_t size, std::future<void>& fu)
                : BasicSizedBuff(data, size)
            {
                fu = mPromise.get_future();
            }
            ...
            virtual void resizeBuffer(uint64_t) {}
        };
        ...
        public:
            ...
            virtual void resizeBuffer(uint64_t size) override
            {
                channelBuffResize(mObj, size);
                set((uint8_t*)channelBuffData(mObj), channelBuffSize(mObj));
            }
        };
        ...
        public:
            ...
            void asyncPerform(ChannelBase* base, io_completion_handle&& completionHandle) override
            {
                ...
                T::asyncPerform(base, [this, base](const error_code& ec, uint64_t bytes) mutable
                    {
        ...
        public:
            ...
            void asyncPerform(ChannelBase* base, io_completion_handle&& completionHandle) override
            {
                ...
                T::asyncPerform(base, [this](const error_code& ec, uint64_t bytes) mutable
                    {
